...
class visual_loss(torch.nn.Module):
    ...
    def forward(self, input:torch.tensor, target:torch.tensor, team_no:int):
        """
        ...
        if len(input.shape) == 4:
            ...
            for i in range(2,input.shape[0]):
                ...
                input_rewards = torch.cat((input_reward[None], input_reward_new[None]))
                target_reward = torch.cat((target_reward[None], target_reward_new[None]))
...
def one_trajectory_run(device, vision_model, action_model, loss, optimizer, train_logger, n_steps, global_step):
    ...
    o = pystk.PlayerConfig(controller = pystk.PlayerConfig.Controller.AI_CONTROL, kart='hexley', team = 0)
    ...
    try:
        ...
        for n in range(1, n_steps):
            input_heatmap = heatmap.clone() #heatmap from past step
            combined_image = torch.cat((resized_image, heatmap), 1) #resized_image from past step
            ...
            l = loss(input_heatmap.squeeze(), heatmap.squeeze(), team_no)
...
def train(args):
    ...
    for e in range(args.epochs):
        ...
        try:
            ...
            heatmap, resized_image = vision_model(img.to(device))
            for n in range(1, n_steps):
                input_heatmap = heatmap.clone().detach() #heatmap from past step
                resized_image = resized_image.detach()
